...
export function CheckoutPage() {
  ...
  return (
    <div className="page-shell relative overflow-hidden">
      <div className="fixed inset-0 z-0 bg-[radial-gradient(ellipse_at_18%_45%,rgba(139,32,56,0.18),transparent_48%),radial-gradient(ellipse_at_82%_15%,rgba(255,255,255,0.06),transparent_36%),#050505]" />
      <Navbar backLink />

      <main className="relative z-10 mx-auto grid min-h-screen max-w-[1720px] gap-10 px-5 pb-20 pt-28 md:px-10 lg:grid-cols-[0.78fr_1fr] lg:pt-32 xl:grid-cols-[0.78fr_1fr_0.78fr] xl:px-16">
        <motion.aside
        ...
        >
          ...
          <div className="h-[300px] overflow-hidden bg-white/[0.025] ring-1 ring-white/[0.055] md:h-[420px]">
            <WatchModel
          ...
          <div className="glass-panel mt-7 p-7 md:p-8">
            <h1 className="font-serif text-[clamp(2.4rem,4vw,3.9rem)] font-semibold leading-none text-white">
              {checkout.model}
            </h1>
            <div className="mt-6 flex flex-col gap-3 border-b border-white/[0.08] pb-6 sm:flex-row sm:items-center sm:justify-between">
              <p className="font-serif text-4xl font-semibold text-white">{checkout.formattedPrice}</p>
              <span className="w-fit bg-ember/20 px-4 py-2 text-[0.68rem] font-bold uppercase tracking-[0.18em] text-emberLight">
                In Stock
              </span>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {[
                ...
                <div key={label} className="flex justify-between gap-5 text-sm text-white/[0.48]">
                  <span>{label}</span>
                  <strong className="text-right font-medium text-white/[0.82]">{value}</strong>
                </div>
              ))}
            </div>
          </div>
        ...
        <motion.form
          ...
          className="min-w-0"
        >
          <h2 className="display-title text-[clamp(2.8rem,5vw,5rem)]">Complete Your Order</h2>
          <p className="mt-4 max-w-xl text-sm leading-7 text-white/[0.55]">
            Secure checkout. All transactions are encrypted and protected.
          </p>

          <section className="glass-panel mt-9 p-6 md:p-8">
          ...
          <button
            type="submit"
            className="relative mt-7 w-full overflow-hidden bg-ember px-7 py-5 text-sm font-bold uppercase tracking-[0.22em] text-white transition duration-300 hover:-translate-y-0.5 hover:bg-emberLight"
          >
          ...
          <p className="mt-5 text-center text-xs tracking-[0.05em] text-white/[0.38]">
        ...
        >
          ...
          <h2 className="font-serif text-[clamp(2.4rem,3vw,3.5rem)] font-semibold leading-none text-white">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-1">
            {checkoutFeatures.map((feature) => (
              <article
                key={feature.number}
                className="group relative min-h-36 overflow-hidden p-6"
                style={{ backgroundImage: `url(${feature.image})`, backgroundSize: "cover", backgroundPosition: "center" }}
              >
                <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,0.88),rgba(0,0,0,0.45))] transition duration-500 group-hover:bg-[linear-gradient(90deg,rgba(0,0,0,0.78),rgba(0,0,0,0.28))]" />
                <div className="relative z-10">
                  <span className="text-[0.65rem] font-bold uppercase tracking-[0.24em] text-emberLight">{feature.number}</span>
                  <h3 className="mt-3 font-serif text-3xl font-semibold leading-none text-white">{feature.title}</h3>
                  <p className="mt-3 max-w-sm text-sm leading-7 text-white/[0.74]">{feature.description}</p>
                </div>
              </article>
            ))}
          </div>
